...
private static boolean debag = false;
	...
	public static void start(StringBuilder ln) {
		...
		findmean(line);
		if(getMean() == 0) {
			throw new NullPointerException("Line have nott element or size of line is zero");
		}
	...
	static public void printline() {	
		...
		System.out.println("Lines are bigger than average: ");
		for(StringBuilder n : line) {
			if(n.length() >= mean) {
				System.out.println(n+"\nKenght of line:" + n.length());
			}
		}
		System.out.println("Lines smaller than average: ");
		for(StringBuilder n : line) {
			if(n.length() < mean) {
				System.out.println(n +"\nLenght of line:" + n.length());
				}
		}
	}
